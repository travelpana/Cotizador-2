...
export function buildDescriptivoPreviewHtml(d: DescriptivoLocal): string {
  ...
  const recomendaciones = recText ? `<div style="margin-top:8px;padding:10px 12px;background:#eff6ff;border-left:3px solid ${COLOR_AZUL};border-radius:6px;"><div style="font-size:10px;font-weight:bold;color:${COLOR_AZUL};letter-spacing:0.6px;text-transform:uppercase;margin-bottom:4px;">Recomendaciones</div><div style="font-size:11px;color:${COLOR_TEXTO};line-height:1.5;">${escNL(recText)}</div></div>` : "";
